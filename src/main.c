...
sb_percentile_t local_percentile_pm;
...
*/
  ...
  if (sb_percentile_init(&local_percentile, 100000, 1.0, 1e13))
    return NULL;
  if (sb_percentile_init(&local_percentile_pm, 100000, 1.0, 1e13))
    return NULL;
  ...
  for( i=0; i < num_conn; i++ ){
      stmt[i] = malloc( sizeof(MYSQL_STMT *) * 43 );
  }
  ...
  for ( i=0; i<6; i++ ){
    if (i == 0) {
    // NO
    printf("  [%d] sc:%d lt:%d  rt:%d  fl:%d NOavg_rt: %.1f (%d)\n",
      i, success[i], late[i], retry[i], failure[i],
      total_rt[i] / (success[i] + late[i]), rt_limit[i]);
  }
  else if (i == 1) {
    // PM
    printf("  [%d] sc:%d lt:%d  rt:%d  fl:%d PMavg_rt: %.1f (%d)\n",
      i, success[i], late[i], retry[i], failure[i],
      total_rt[i] / (success[i] + late[i]), rt_limit[i]);
  }
  else if (i == 5) {
    // PM
    printf("  [%d] sc:%d lt:%d  rt:%d  fl:%d CRavg_rt: %.1f (%d)\n",
      i, success[i], late[i], retry[i], failure[i],
      total_rt[i] / (success[i] + late[i]), rt_limit[i]);
  }
  else {
  ...
  for ( i=0; i<6; i++ ){
    if (i == 0) {
      // NO
      printf("  [%d] sc:%d  lt:%d  NOrt:%d  fl:%d \n", i, success2_sum[i], late2_sum[i], retry2_sum[i], failure2_sum[i]);
    }
    else if (i == 1) {
      // PM
      printf("  [%d] sc:%d  lt:%d  PMrt:%d  fl:%d \n", i, success2_sum[i], late2_sum[i], retry2_sum[i], failure2_sum[i]);
    }
    else if (i == 5) {
      // PM
      printf("  [%d] sc:%d  lt:%d  CRrt:%d  fl:%d \n", i, success2_sum[i], late2_sum[i], retry2_sum[i], failure2_sum[i]);
    }
    else {
      // printf("  [%d] sc:%d  lt:%d  rt:%d  fl:%d \n", i, success2_sum[i], late2_sum[i], retry2_sum[i], failure2_sum[i]);
      continue;
  ...
  f = 100.0 * (float)(success[5] + late[5])/(float)j;
  printf("    Credit-Check: %3.2f%% (>= 4.0%%)",f);
  if ( f >= 4.0 ){
    printf(" [OK]\n");
  }else{
    printf(" [NG] *\n");
  ...
  f = 100.0 * (float)success[5]/(float)(success[5] + late[5]);
  printf("    Credit-Check: %3.2f%% ",f);
  if ( f >= 90.0 ){
    printf(" [OK]\n");
  }else{
    printf(" [NG] *\n");
...
void alarm_handler(int signum)
{
  ...
  double percentile_val90;
  double percentile_val_pm;
  double percentile_val99_pm;
  double percentile_val90_pm;
  ...
  percentile_val90 = sb_percentile_calculate(&local_percentile, 90);
  percentile_val = sb_percentile_calculate(&local_percentile, 95);
  percentile_val99 = sb_percentile_calculate(&local_percentile, 99);
  percentile_val90_pm = sb_percentile_calculate(&local_percentile_pm, 90);
  percentile_val_pm = sb_percentile_calculate(&local_percentile_pm, 95);
  percentile_val99_pm = sb_percentile_calculate(&local_percentile_pm, 99);
  sb_percentile_reset(&local_percentile);
//  printf("%4d, %d:%.3f|%.3f(%.3f), %d:%.3f|%.3f(%.3f), %d:%.3f|%.3f(%.3f), %d:%.3f|%.3f(%.3f), %d:%.3f|%.3f(%.3f)\n",
  // printf("%4d, trx: %d, 95%: %.3f, 99%: %.3f, max_rt: %.3f, %d|%.3f, %d|%.3f, %d|%.3f, %d|%.3f, %d %d\n",
	//  time_count,
	...
	//  (double)cur_max_rt[4], retry[0]-prev_rt0, retry[1]-prev_rt1
	//  );
  //  prev_rt0 = retry[0];
  //  prev_rt1 = retry[1];

  printf("%4d, NOtrx:%d, NOp:%.3f,%.3f,%.3f, PMtrx:%d, PMp:%.3f,%.3f,%.3f, CRtrx:%d, %d %d\n",
  time_count,
  ( s[0] + l[0] - prev_s[0] - prev_l[0] ), percentile_val90, percentile_val,percentile_val99,
  ( s[1] + l[1] - prev_s[1] - prev_l[1] ), percentile_val90_pm, percentile_val_pm,percentile_val99_pm,
  ( s[5] + l[5] - prev_s[5] - prev_l[5] ),
  retry[0]-prev_rt0, retry[1]-prev_rt1
  );
  prev_rt0 = retry[0];
  prev_rt1 = retry[1];
...
void alarm_dummy()
{
  ...
  time_count += PRINT_INTERVAL;
  printf("%4d, %d(%d):%.2f, %d(%d):%.2f, %d(%d):%.2f, %d(%d):%.2f, %d(%d):%.2f, %d(%d):%.2f\n",
	 time_count,
...
int thread_main (thread_arg* arg)
{
  ...
  if( mysql_stmt_prepare(stmt[t_num][9], "UPDATE warehouse SET w_ytd = w_ytd + ? WHERE w_id = ?", 53) ) goto sqlerr;
  if( mysql_stmt_prepare(stmt[t_num][10], "SELECT w_street_1, w_street_2, w_city, w_state, w_zip, w_name FROM warehouse WHERE w_id = ? FOR UPDATE", 102) ) goto sqlerr;
  if( mysql_stmt_prepare(stmt[t_num][11], "UPDATE district SET d_ytd = d_ytd + ? WHERE d_w_id = ? AND d_id = ?", 67) ) goto sqlerr;
  if( mysql_stmt_prepare(stmt[t_num][12], "SELECT d_street_1, d_street_2, d_city, d_state, d_zip, d_name FROM district WHERE d_w_id = ? AND d_id = ? FOR UPDATE", 116) ) goto sqlerr;
  ...
  if( mysql_stmt_prepare(stmt[t_num][35], "SELECT c_balance, c_credit_lim FROM customer WHERE c_id = ? AND c_d_id = ? AND c_w_id = ?", 89) ) goto sqlerr;
  if( mysql_stmt_prepare(stmt[t_num][36], "SELECT SUM(ol_amount) FROM order_line, orders, new_orders WHERE ol_o_id = o_id AND ol_d_id = ? AND ol_w_id = ? AND o_d_id = ? AND o_w_id = ? AND o_c_id = ? AND no_o_id = o_id AND no_d_id = ? AND no_w_id = ?", 206) ) goto sqlerr;
  if( mysql_stmt_prepare(stmt[t_num][37], "UPDATE customer SET c_credit = ? WHERE c_id = ? AND c_d_id = ? AND c_w_id = ?", 77) ) goto sqlerr;
